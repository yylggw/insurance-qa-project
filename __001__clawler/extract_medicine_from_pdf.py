...
# -*- coding: utf-8 -*-
"""
医保药品目录PDF提取工具（仅抽取数据，不写入Neo4j）
功能：将10363页PDF解析为19列CSV文件
"""
...
#  👇 1. 只需修改这里的 PDF 文件路径
# =========================================================
PDF_PATH = "../__000__data/药品-医保药品分类与代码数据(西药、中成药)截至2026年7月24日.pdf"  # 改成你下载的73MB文件路径
# =========================================================

# 输出配置
OUTPUT_CSV = "medicine_full_data.csv"  # 最终输出的CSV文件名
TEMP_DIR = "temp_all_medicine_csv"  # 临时文件目录（防止内存爆掉）
BATCH_SIZE = 200  # 每200页存一次临时文件

# 运行模式配置
TEST_MODE = False  # True=只跑前10页测试；False=跑全量
...
def clean_row(row):
    ...
    # 跳过表头行（第一列包含"药品代码"或"项目内容"）
    if cleaned[0] and ("药品代码" in cleaned[0] or "项目内容" in cleaned[0]):
        return None
...
def main():
    ...
    if final_csv is not None:
        ...
        print("\n💡 下一步建议：")
        print("   1. 用Excel或文本编辑器打开CSV，检查前几行数据是否正确")
        print("   2. 确认无误后，告诉我数据质量，我帮你生成清洗和入库脚本")
